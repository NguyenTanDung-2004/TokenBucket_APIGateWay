...
import com.example.APIGateway.service.CartService;
...
public class AuthenticationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        System.out.println("<API Gateway - Port: 8888> Entering filterchain...");

        // check public point
        if (isPublicEndPoints(exchange.getRequest())) {
            System.out.println("<API Gateway - Port: 8888> Public EndPoint");
                ...
                return userService.checkToken("4567").flatMap(numberOfTokenResponse -> {
                    ...
                    if ((Integer) numberOfTokenResponse.getBody().get("Code") == 1000) {
                        ...
                        return this.cartService.addToCart().flatMap(cartServiceResponse -> {
                            ...
                            System.out.println("Error getting number of tokens: " + throwable.getMessage());
                            return unauthenticated(exchange.getResponse());
                        });
                    } else {
